...
- **Categories are discovered from the filesystem**: `engine.list_categories` globs `problems/*` for directories and sorts them, so adding `problems/<category>/` is all it takes to make a category selectable — no list to update.
