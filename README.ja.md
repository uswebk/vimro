...
2. 表示言語（`ja` / `en`）とカテゴリを選択します（MVP は `plain` のみ）。
3. 画面が左右に分割されます。**右**ペインのお題どおりに**左**バッファを編集してください。
4. クリアは自動で判定されます。右ペインで `n` を押すと次の問題へ。
...
同じ操作は**左の練習バッファからも**プレフィックス付きで実行できます（ペイン移動不要）: `<leader>n` で次へ、`<leader>r` でリセットなど。`<leader>` の実キーは環境によって異なり（LazyVim なら Space、未設定なら `\`）、右ペインの案内には解決済みの実キー名が表示されます。素の `n` や `r` は練習バッファでは vim 本来の動きのままです — それを練習するためのプラグインなので。プレフィックスは `practice_prefix` で変更でき、`false` で無効化できます。
