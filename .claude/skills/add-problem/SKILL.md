...
リポジトリ常設のスクリプトをリポジトリルートで実行する:

```sh
nvim --headless -u NONE -l scripts/verify_problems.lua
```

これで全問題について次を機械確認する（同じものが CI でも走る）:

- **全 solutions の keys が start を goal に本当に変形するか**（headless 再生）
- `id` の形式（`<category>-NNN`）と重複
- `notes` の要素数が `solutions` と一致（ja / en 両方）、`i18n.ja` / `i18n.en` の必須フィールド
- `optimal: true` がちょうど1つ、`difficulty` が 1〜3

失敗すると該当 `id` と実際に生成されたバッファ内容が出るので、`start` / `goal` / `keys` を見直す。

これに加えて **`description` と `goal` が矛盾していないか**だけは目視で確認する（機械検証できない）。
